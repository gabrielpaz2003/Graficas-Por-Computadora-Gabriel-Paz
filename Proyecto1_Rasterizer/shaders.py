...
from mathl import dotProduct, normalize, reflect, MatrixProduct, MatrixVectorProduct
import random
...
def metallicShader(**kwargs):
    ...
    reflectDir = [2 * normal[i] * sum([normal[j] * dirLight[j]
                                      for j in range(3)]) - dirLight[i] for i in range(3)]
    reflectDir = normalize(reflectDir)

    spec = max(0, sum([reflectDir[i] * cameraDir[i] for i in range(3)])) ** 16

    if len(textureList) > 0:
        texColor = textureList[0].getColor(u, v)
        r = texColor[0] * 0.6 + spec * 0.4
        g = texColor[1] * 0.6 + spec * 0.4
        b = texColor[2] * 0.6 + spec * 0.4
    else:
        r = g = b = spec
